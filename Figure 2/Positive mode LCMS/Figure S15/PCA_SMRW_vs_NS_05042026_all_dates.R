...
#make scree plot 
res.pca <- data_merge  %>% dplyr::select(-ATTRIBUTE_Sample_Month_Collected) %>% 
  prcomp(scale = FALSE) #TRUE if scaling was not performed before
fviz_eig(res.pca)

#plot pca biplot
biplot <- fviz_pca_biplot(res.pca, repel = TRUE, 
                col.ind = data_merge$ATTRIBUTE_Sample_Month_Collected, #individual color
                label = "var",
                addEllipses = TRUE,
...
#plot pca plot
pca <- fviz_pca_ind(res.pca, col.ind = data_merge$ATTRIBUTE_Sample_Month_Collected, 
