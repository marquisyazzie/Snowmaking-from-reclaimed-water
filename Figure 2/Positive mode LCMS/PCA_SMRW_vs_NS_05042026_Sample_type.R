...
metadata <- read.csv("avg_tech_reps_metadata_05222024_pos_mode.csv")
...
#data_merge <- metadata %>% dplyr::select("filename", "ATTRIBUTE_Sample_Month_Collected") %>% 
  left_join(clr) %>% 
  column_to_rownames("filename")
...
write.csv(permanova_all, "Permanova results SMRW vs NS 05042026.csv", row.names=TRUE)
...
#plot pca plot
pca <- fviz_pca_ind(res.pca, col.ind = data_merge$ATTRIBUTE_All_Sample_Type, 
...
ggsave(file="pca_SMRW_vs_NS_05062026_all_sample_type.svg", plot=new_pca, width=6, height=4)
