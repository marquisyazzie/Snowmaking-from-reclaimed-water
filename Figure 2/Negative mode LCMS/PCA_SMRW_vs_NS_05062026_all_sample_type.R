setwd("C:/Users/Marquis.Yazzie/OneDrive - University of Denver/Desktop/Aron_Lab_Data/SMRW_vs_NS_project/PCA/negative SMRW v NS")
...
metadata <- read.csv("metadata_neg_mode_10162023_SMRW_vs_NS.csv")
clr <- read.csv("mzmine3_10162023_neg_CLR_Scaled_SMRW_vs_NS.csv")

data_merge <- metadata %>% dplyr::select("filename", "ATTRIBUTE_All_Sample_Type") %>% 
  left_join(clr) %>% 
  column_to_rownames("filename")

# filter data in sample type column
data_merge <- data_merge %>% filter(ATTRIBUTE_All_Sample_Type != "Ext_Blank")

data_merge <- data_merge %>% filter(ATTRIBUTE_All_Sample_Type != "Pooled_QC")

  

##specify the number of colors needed for snow type variable
#num_colors <- length(unique(data_merge$ATTRIBUTE_Sample_Type))

#choose color blind pallete
#color_palette <- brewer.pal(num_colors, "Set1")

#make scree plot 
res.pca <- data_merge  %>% dplyr::select(-ATTRIBUTE_All_Sample_Type) %>% 
...
write.csv(permanova_all, "Permanova results SMRW vs NS neg 12182024 no QC.csv", row.names=TRUE)
...
pca <- fviz_pca_ind(
  res.pca,
  col.ind = data_merge$ATTRIBUTE_All_Sample_Type,
  addEllipses = TRUE,
  label = "none",
  legend.title = "",
  palette = c("SMRW" = "#F8766D", "NS" = "#619CFF")
)
...
new_pca <- pca + 
  labs(title = "", # Add a descriptive title
       x = "Principal Component 1 (37%)", # Replace with the actual explained variance
       y = "Principal Component 2 (21%)") +
  theme_minimal(base_size=12) + # Adjust the theme if needed
  theme(legend.position = "right",  # Adjust legend position if needed
        panel.grid = element_blank())
new_pca

ggsave(file="pca_all_sample_type_negative_05062026.svg", plot=new_pca, width=6, height=4)
