...
data_merge <- metadata %>% dplyr::select("filename", "ATTRIBUTE_Date_Collected") %>% 
  left_join(clr_metals) %>% 
  column_to_rownames("filename")

# filter data in sample type column
data_merge <- data_merge %>% filter(Sample_type %in% c("NS_March_2022", "NS_SMRW_2022"))
...
#make scree plot 
res.pca <- data_merge  %>% dplyr::select(-ATTRIBUTE_Date_Collected) %>% 
...
#plot pca plot
pca <- fviz_pca_ind(res.pca, col.ind = data_merge$ATTRIBUTE_Date_Collected, 
                    addEllipses = TRUE, label = "none", 
                    legend.title= "Date_Collected")
pca
...
ggsave(file="pca_05042026_alldates.svg", plot=new_pca, width=10, height=8)
